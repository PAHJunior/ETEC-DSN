...
INSERT INTO tbl_Livros (nome_livro, data_publicacao, paginas, valor_unitario, FK_Editora) VALUES
    ('Meu Ultimo livro', '2019/05/24', 125, 85.99, 5);
...
#  Exercicio 1
select nome_autor, nasc_autor, nacionalidade from tbl_autor where codigo_autor in (9,10);
#  Exercicio 2

create view vw_exer2 as
...
create view vw_primeira as
	select * from tbl_livros;

desc vw_primeira;
	select nome_livro from vw_primeira where codigo_livro = 10;


select a.nome_autor, count(*) from autor_livro d
join tbl_autor a on d.fk_autor = a.codigo_autor
join tbl_livros b on d.fk_livro = b.codigo_livro
join tbl_categoria c on b.fk_categoria = c.codigo_categoria
where c.nome_categoria in ("romance") group by a.nome_autor;

#  Exercicio 6
select b.nome_autor, c.nome_livro from autor_livro a
join tbl_autor b on b.codigo_autor = a.fk_autor
join tbl_livros c on c.codigo_livro = a.fk_livro;


# Exercicio 7
select a.nome_autor, count(*) as total_livro from autor_livro d
join tbl_autor a on d.fk_autor = a.codigo_autor
join tbl_livros b on d.fk_livro = b.codigo_livro
join tbl_categoria c on b.fk_categoria = c.codigo_categoria
group by a.nome_autor order by total_livro desc;


# Exercicio 8
select nome_livro, valor_unitario from tbl_livros where valor_unitario > (select avg(valor_unitario) from tbl_livros) order by valor_unitario;


# Exercicio 9
select nome_livro, paginas, b.nome_categoria from tbl_livros a
join tbl_categoria b on a.fk_categoria = b.codigo_categoria where paginas < (select avg(paginas )from tbl_livros);


# Exercicio 10
select nome_livro, b.nome_editora, d.retirada from tbl_livros a
join tbl_editora b on a.FK_editora = b.codigo_editora
join livro_emprestimo c on a.codigo_livro = c.fk_livro
join tbl_emprestimo d on c.fk_emprestimo = d.codigo_emprestimo where d.retirada > (now() - interval 6 month);
