...
#DROP DATABASE PAULO_ARTHUR_HALLER_JUNIOR;
# 1 - CRIE O BANCO DE DADOS COM O SEU NOME.
CREATE DATABASE PAULO_ARTHUR_HALLER_JUNIOR;

use PAULO_ARTHUR_HALLER_JUNIOR;
...
select * from professor where nome_Professor = 'Aline';


##update nome_professor;
...
select nome_aluno from aluno where nome_aluno like '%silva%' order by nome_aluno desc;
select * from aluno;
...
select count(*) from aluno where nascimento between '1990-01-01' and '2000-12-31';

# 5 - SELECIONAR QUANTOS ALUNOS ESTÃO INSCRITOS EM CADA UM DOS CURSOS E RENOMEIE AS COLUNAS DO SELECT. DEIXE O CODIGO ABAIXO:
select count(fk_id_curso) as Quantidade_De_Alunos_Em_Cada_Curso from aluno_curso group by fk_id_curso;
...
select * from nota;
select sum(nota) as Soma_Notas, avg(nota) as Media_Nota, min(nota) as Min_nota from nota;


# 7 - EXIBIR QUAIS OS TITULOS E AS EXPLICACÕES PARA OS TOPICOS DO CURSO DE "BANCO DE DADOS".
select * from secao;
select * from curso;
select * from secao where fk_id_curso = 1;
...
select * from aluno;
select * from aluno where nascimento < '2000-12-31' order by nascimento desc, nome_aluno;